...
def sample_energy_demand_data() -> None:
    np.random.seed(42)

    dates = pd.date_range(start='2024-01-01', end='2024-12-31', freq='h')

    demand = 1000 + 200 * np.sin(np.arange(len(dates)) * 2 * np.pi / 24) + 100 * np.sin(np.arange(len(dates)) * 2 * np.pi / (24 * 7)) + np.random.normal(0, 50, len(dates))
...
def generate_data_3() -> None:
    ...
    print(prophet_results[prophet_results['ds'] > df['timestamp'].max()][['ds', 'yhat', 'yhat_lower', 'yhat_upper']].describe())


def generate_data_4() -> None:
    np.random.seed(42)
...
def create_figure_with_subplots() -> None:
    import matplotlib.pyplot as plt
    import numpy as np
    import pandas as pd
    import seaborn as sns
