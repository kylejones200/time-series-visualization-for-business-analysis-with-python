...
def main():
    parser = argparse.ArgumentParser(
        description="Time Series Visualization for Business Analysis"
    )
    ...
    if args.data_path and args.data_path.exists():
        ...
        values = (
            np.sin(np.arange(config["data"]["n_periods"]) / 30) * 100
            + 500
    ...
    if config["visualization"]["seasonal_decomposition"]:
        plot_seasonal_decomposition(
            df,
            "value",
            config["visualization"]["seasonal_period"],
            "Seasonal Decomposition",
            output_dir / "seasonal_decomposition.png",
        )
