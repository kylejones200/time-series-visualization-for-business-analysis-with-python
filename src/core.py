...
def plot_time_series(df: pd.DataFrame, value_col: str, date_col: str, title: str, output_path: Path, plot: bool=False):
    """Plot time series"""
    if not plot:
        return
...
def plot_multiple_series(df: pd.DataFrame, columns: list[str], title: str, output_path: Path, plot: bool=False):
    ...
    for i, col in enumerate(columns):
        ax.plot(df.index if hasattr(df.index, '__len__') else range(len(df)), df[col], label=col, color=colors[i % len(colors)], linewidth=1.2)
    ax.set_xlabel('Time')
    ax.set_ylabel('Value')
    ax.legend(loc='best')
    plt.savefig(output_path, dpi=100, bbox_inches='tight')
...
def plot_seasonal_decomposition(df: pd.DataFrame, value_col: str, period: int, title: str, output_path: Path, plot: bool=False):
